...
const Timeline = () => {
  ...
  const slideTo = (index) => {
    const safeIndex = Math.max(0, Math.min(index, 6)); // 8 slides, index range is 0-7
  ...
  return (
    <div id='TimelineBigDiv' ref={carouselRef} className="relative bg-black h-screen overflow-hidden">
      ...
      >
        ...
        {/* Slide 7 - Outro */}
        <motion.div 
          className="min-w-full h-screen flex flex-col justify-center bg-cover bg-center font-customTwo" 
          style={{ backgroundImage: `url(${landingOne})` }}>
            
            <div className="text-white font-bold mx-12">
              <motion.div className="h-full flex flex-col justify-center bg-black bg-opacity-40 p-12"> 
                <div className='text-6xl neon-glow leading-tight'>
                I believe in leading with a collaborative spirit, and I approach every project with empathy, creativity, organization, and a keen eye for detail. 
                My goal is always to inspire and lead, fostering a team-oriented environment!
                </div>
              </motion.div>
            </div>
        </motion.div>
